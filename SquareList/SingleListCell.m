...
- (void)setListItem:(ListItem *)listItem
{
    ...
    if (listItem) {
        ...
        self.textView.textColor = (listItem.list.textColor.integerValue == 0) ? [UIColor whiteColor] : [UIColor blackColor];
    }
}
...
- (void)setColor:(UIColor *)color
{
    ...
    self.deleteLabel.textColor = self.color;
}
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.deleteLabel.backgroundColor = [Colors interactiveColor];
        self.deleteLabel.text = @"Delete";
        self.deleteLabel.textColor = self.color;
        self.deleteLabel.textAlignment = NSTextAlignmentCenter;
        self.deleteLabel.font = [Fonts bodyTextFont];
...
- (void)cellPressed:(UILongPressGestureRecognizer *)sender
{
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        [UIView animateWithDuration:0.35
                            ...
                            options:UIViewAnimationOptionCurveEaseInOut
                         animations:^{
                             CGPoint newOrigin = [self.delegate newOriginForMovedCell:self];
                             cell.frame = CGRectMake(newOrigin.x, newOrigin.y, cell.frame.size.width, cell.frame.size.height);
