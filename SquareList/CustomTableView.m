...
- (void)moveCell:(UITableViewCell *)cell toIndex:(NSInteger)index animated:(BOOL)animated completion:(void (^)())completionBlock
{
    ...
    CGFloat newY;
    if (rowForCell == index-1 || rowForCell == index+1) {
        newY = (rowForCell > index) ? cell.frame.origin.y - cellToMoveTo.frame.size.height : cell.frame.origin.y + cellToMoveTo.frame.size.height;
    } else {
        newY = cellToMoveTo.frame.origin.y;
    }
...
{
    if (index == 0)return CGPointMake(0.0, 0.0);
    UITableViewCell *preceedingCell = [self.cells objectAtIndex:index - 1];
    return CGPointMake(0.0, preceedingCell.frame.origin.y + preceedingCell.frame.size.height);
}
